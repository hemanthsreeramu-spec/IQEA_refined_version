            ...
            if option == 'Recorded_Details':
                if st.session_state.selected_images and prompt:
                    ...
                    if model_type == "azureopenai":
                        ...
                        constructedprompt = utils.generate_pom_from_excel_testcases("Test_case_generation_gemini",
                                                                                    navigation,
                                                                                    image_data_processed, Action_data,
                                                                                    prompt)
                        print("******final prompt *******")
                        ...
                        if st.session_state.azure_workitem_id:
                            try:
                                ...
                                if exists:
                                    ...
                                    st.success(
                                        f"✅ Test cases were successfully exported!\n\n📄 Work Item ID: **{child_id}**")
                                else:
    ...
    if st.session_state.checkbox6_state:
        ...
        with st.expander("🧾 Test Automation Script Generator"):
            ...
            test_file_language = st.selectbox("Select Language for test file",
                                              ["Java-Selenium", "Java-Playwright", "Python-Selenium",
                                               "Python-Playwright", "Desktop & Web[python-Selenium]",
                                               "UTAM-JavaScript"])
            if test_file_language != "Desktop & Web[python-Selenium]":
                ...
                if source == "file":
                    ...
                    Action_data = utils.select_and_read_text_files_xpath("recorded action (Optional)",
                                                                         Action_collection)
                elif source == "database":
                ...
                if st.button("Generate_Test_Script"):
                    ...
                    # script_validator_prompt = utils.generate_script_validator(Lang_lib_validate, page_files_content,
                    ...
                    if source == "file":
                        utils.create_test_file(Test_file_generator, test_file_name, test_file_language,
                                               test_script_response)
                    elif source == "database":
                        db_handler.save_testfile_to_db(test_file_name, test_script_response, get_update_user(),
                                                       test_file_language)
            else:
                ...
                if st.button("Generate_Test_Script"):
                    Lang_lib = "Test-" + test_file_language
                    Prompt = utils.generate_test_script_windows_web(Lang_lib, Action_data,
                                                                    test_windows_application_path)
                    test_script_response = utils.get_queries_from_ai_updated(Prompt)
                    if source == "file":
                        utils.create_test_file(Test_file_generator, test_file_name, test_file_language,
                                               test_script_response)
