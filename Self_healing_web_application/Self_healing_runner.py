...
# feature_file_location=healing_framework_utils.write_feature_file("full_workflow", feature_content)


# #####MCP - Rweady feature , generate JS file
# asyncio.run(healing_utils.main(feature_file_location))
# #### Subprocess to run genearted JS file to get DOM#############
# #subprocess.run(['node', healing_utils.SCRIPT_PATH_execution], check=True)
# subprocess.run(['node', healing_utils.SCRIPT_PATH], check=True)
...
# dom_file_path =r"D:\Self_healing_web_application\output\Dom_details\all_page_dom_details.json"
